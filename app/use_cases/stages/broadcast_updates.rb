class Stages::BroadcastUpdates
  ...
  def self.call(stage, filter_status_deal)
    ...
    stage.broadcast_replace_to :stages, 
      target: dom_id(stage, filter_status_deal),
      partial: 'accounts/stages/stage', 
      locals: { 
        filter_status_deal:,
        pagy: pagy_filtered_deals, 
        deals: stage_filtered_deals,
        account: stage.account 
      }

    stage.broadcast_replace_to :stages, 
      target: dom_id(stage, 'all'),
      partial: 'accounts/stages/stage', 
      locals: { 
        filter_status_deal: 'all', 
        pagy: pagy_all_deals, 
        deals: stage_all_deals,
        account: stage.account 
      }
  end
end
